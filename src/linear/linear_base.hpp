...
namespace math
{
    ...
    template <typename SELF, size_t DIM>
        requires is_power_of_2<DIM>
    ...
    /// @brief Negate the linear object by entry-wise nagation.
    template <typename VEC, size_t DIM>
    inline VEC operator-(const linear_base<VEC, DIM> &right)
    {
        return -static_cast<linear_base<VEC, DIM>::num_vec>(right);
    }
    ...
    {
        return static_cast<linear_base<VEC, DIM>::num_vec>(left) + static_cast<linear_base<VEC, DIM>::num_vec>(right);
    }
